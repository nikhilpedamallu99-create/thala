...
import { FileText, Trash2, CheckCircle2, Clock, AlertTriangle, Layers, RefreshCw, Eye } from 'lucide-react';
  ...
  return (
    <>
      <div className="p-4 rounded-xl glass-card flex flex-col sm:flex-row sm:items-center justify-between gap-4 border transition-all">
          ...
          <div>
            ...
            <div className="flex flex-wrap items-center gap-3 text-xs text-slate-400 mt-1">
              ...
              <button
                ...
                className="flex items-center gap-1 text-indigo-400 hover:text-indigo-300 hover:underline font-medium cursor-pointer"
        ...
        <div className="flex items-center justify-between sm:justify-end gap-2.5 pt-3 sm:pt-0 border-t sm:border-0 border-slate-800/80">
          {getStatusBadge(doc.status)}

          {doc.status === 'Ready' && (
            <button
              onClick={() => setShowChunksModal(true)}
              className="px-3 py-1.5 rounded-lg bg-cyan-500/10 hover:bg-cyan-500/20 text-cyan-300 border border-cyan-500/30 transition-all text-xs flex items-center gap-1.5 font-medium"
              title="View document vector chunks"
            >
